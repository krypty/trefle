...
py::array_t<double> predict_c(py_array_f ind, py_array_d observations,
                              const int n_rules, const int max_vars_per_rule,
                              const int n_labels, const int n_consequents,
                              py_array_i default_rule_cons,
  ...
  double *y_preds_ptr = fiseval.predict(
      (float *)buf_ind.ptr, buf_ind.size, (double *)buf_observations.ptr,
      buf_observations.shape[0], buf_observations.shape[1], n_rules,
      max_vars_per_rule, n_labels, n_consequents,
      (int *)buf_default_rule_cons.ptr, buf_default_rule_cons.size,
      (double *)buf_vars_ranges.ptr, buf_vars_ranges.shape[0],
      buf_vars_ranges.shape[1], (double *)buf_labels_weights.ptr,
      buf_labels_weights.size, dc_idx);
